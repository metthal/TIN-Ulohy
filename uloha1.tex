...
\subtask % (b)
Gramatika $G_{1}$ je typu 2 z dôvodu tvaru prepisovacích pravidiel, ktorý odpovedá $A \to \alpha$, kde $A \in N$ a $\alpha \in (N \cup \Sigma)^{*}$.
Nemôže byť typu 3, pretože pravidlá porušujú tvar $A \to xB$ a $A \to x$, kde $A,B \in N$ a $x \in \Sigma^{*}$.

Typ gramatiky a typ jazyka sa môže všeobecne líšiť. Podľa vety 2.3 zo študíjnej opory platí

\begin{equation*}
    \mathcal{L}_{3} \subseteq \mathcal{L}_{2} \subseteq \mathcal{L}_{1} \subseteq \mathcal{L}_{0}
\end{equation*}

kde $\mathcal{L}_{i}$ je trieda všetkých jazykov typu $i$. Podľa Chomského definícií gramatík v kaptiole 2.3 v študíjnej
opore a z Chomského hierarchie jazykov spomenutej vyššie vyplýva, že každá gramatika typu 3 je zároveň typu 2 a každá gramatika
typu 1 je zároveň typu 0. Z inklúzie $\mathcal{L}_{2} \subseteq \mathcal{L}_{1}$ taktiež platí, že každú gramatiku typu 2 je možné
transformovať na ekvivalentnú gramatiku typu 1. Gramatiky typu $i$ sú tým pádom schopné generovať jazyky typu $i$ a vyššie.

Jazyk $L_{1}$ teda môže byť typu 2 či 3. To vyplýva z toho, že gramatika typu $i$ môže generovať jazyk
typu $i$ a vyšší. Je teda potrebné dokázať, o aký jazyk sa jedná pomocou pumping lemma pre
regulárne jazyky. Predpokladajme, že $L_{1}$ je regulárny jazyk (typu~3).
Potom platí že
\begin{align*}
    \exists p > 0 : \forall w \in L_{1} :
    \vert w \vert \ge p \Rightarrow
    \exists x,y,z \in \Sigma^{*} :&\ w = xyz\ \land \\
                                  &\ y \ne \varepsilon\ \land \\
                                  &\ \vert xy \vert \le p\ \land \\
                                  &\ \forall i \ge 0 : xy^{i}z \in L_{1}\
\end{align*}
...
$xy^{0}z = a^{p - \vert y \vert}b^{j}c^{p}d^{k}$. Nakoľko musí platiť, že
$\#_{a}(a^{p - \vert y \vert}b^{j}c^{p}d^{k}) = \#_{c}(a^{p - \vert y \vert}b^{j}c^{p}d^{k})$,
tak musíme nájsť riešenie rovnice $p - \vert y \vert = p$. Táto rovnica má riešenie jedine pre
$\vert y \vert = 0$. Vieme však, že $y \neq \varepsilon$ a teda $\vert y \vert > 0$.
Dospeli sme ku sporu. Pôvodný predpoklad, že jazyk $L_{1}$ je regulárny (typu~3) je teda neplatný.
Jazyk $L_{1}$ musí byť typu 2.
\end{mysolution}
...
\begin{mysolution}{subtasks}
    ...
    \subtask Relácia $\sim_{L}$ je definovaná definíciou 3.17 v študíjnej opore nasledovne.
    ...
    Podľa vety 3.21 (Myhill-Nerodova veta) zo študíjnej opory taktiež platí, že počet stavov ľubovolného minimálneho DKA príjmajúceho $L$
    je rovný indexu $\sim_{L}$. Tento minimálny automat sme si zostrojili v úlohe (a). Zostáva nám určiť triedy ekvivalencie relácie $\sim_{L}$,
    ktoré odpovedajú triedam ekvivalencie minimálneho DKA $M_{Z}$ z úlohy (a). Pre každú triedu resp. stav zostrojíme jazyk $L^{-1}$,
    odpovedajúci jazyku príjmaného v tomto stave, a k nemu odpovedajúci konečný automat.
    ...
    \begin{figure}[!h]
        ...
        \begin{tikzpicture}[node distance = 2.5cm, ->, >=stealth, line width=1pt]
            ...
            \path (d) edge node[above]{$c$} (f);
            \path (e) edge node[above]{$c$} (g);
            \path (f) edge[bend right=30] node[above]{$a$} (b);
            \path (g) edge[bend left=30] node[below]{$a$} (c);
        ...
        \end{tikzpicture}
        \captionsetup{justification=centering}
        \caption{KA príjmajúci jazyk $L^{-1}([Z]) = \Sigma^{*} \setminus (L^{-1}([A]) \cup L^{-1}([B]) \cup L^{-1}([C]) \cup L^{-1}([D]) \cup L^{-1}([E]) \cup L^{-1}([G]))$.}
    \end{figure}
...
rovnice nad regulárnymi výrazmi v štandardizovanom tvare $X = pX + q$ je $X = p^{*}q$.
\begin{flalign}
    Y =&\ caY + cY + bbaY + baY + b & \\
      =&\ (ca + c + bba + ba)Y + b & \\
      =&\ (ca + c + bba + ba)^{*}b &
    ...
    \begin{flalign*}
        ...
        &\delta_{restrict} :
                \forall q_{1}, q_{2} \in Q_{1} \forall q_{3}, q_{4} \in Q_{2} \forall a \in \Sigma_{1}\forall b \in \Sigma_{2} : & \\
                    ...
                    &\hspace{200pt} \Longleftrightarrow & \\
...
\begin{mysolution}
    ...
    Predpokladajme, že jazyk $L$ je regulárny jazyk.
    Potom platí, že
    \begin{align*}
        \exists p > 0 : \forall w \in L :
        \vert w \vert \ge p \Rightarrow
        \exists x,y,z \in \Sigma^{*} :&\ w = xyz\ \land \\
                                      &\ y \ne \varepsilon\ \land \\
                                      &\ \vert xy \vert \le p\ \land \\
    ...
    výhradne zo symbolov $a$. Môžeme teda zapísať, že $xy^{i}z = a^{p + (i-1)\vert y \vert}b^{2p}c^{j}$.
    Pre $i = 0$ platí, že $xy^{0}z = a^{p - \vert y \vert}b^{2p}c^{j}$. Z toho vyplýva, že
    musí platiť $\#_{b}(a^{p - \vert y \vert}b^{2p}c^{j}) = 2\#_{a}(a^{p - \vert y \vert}b^{2p}c^{j})$.
    Hľadáme teda riešenie rovnice $p - \vert y \vert = 2p$, ktoré je $\vert y \vert = -p$.
    To však platí iba pre $\vert y \vert = 0$, pretože dĺžka reťazca nemôže byť záporná. Dostávame
    sa však do sporu, pretože musí platiť, že $p > 0$ a zároveň musí platiť, že $y \neq \varepsilon$, teda $\vert y \vert \neq 0$.
    Pôvodný predpoklad, že jazyk $L$ je regulárny teda nie je platný. Jazyk $L$ nie je regulárny.
\end{mysolution}
...
\task{6}{Uvažujme algebru regulárnych množín ($A_{RM}$) nad abecedou $\Sigma$.
    \begin{subtasklist}
        ...
        Kleenova algebra.
        \subtask Určite, či je $\subseteq$ \emph{totálne usporiadanie} v $A_{RM}$. Teda, že pre
...
\begin{mysolution}{subtasks}
    ...
    Podľa vety 2.1 zo študíjnej opory taktiež pre iteráciu platí nasledovné.
    \begin{align}
        L^{*} &= L^{+} \cup \{\varepsilon\} \\
        L^{+} &= L.L^{*} = L^{*}.L
    \end{align}
    ...
    \subtask Pokiaľ má byť operácia inklúzie $\subseteq$ \emph{totálnym usporiadaním}, tak musí pre ľubovoľné 2~regulárne množiny
    $A,B$ platiť $A \subseteq B$ alebo $B \subseteq A$. Zvoľme preto ľubovoľnú abecedu $\Sigma = \{ a, b \}$ nad ktorou zostrojíme
    2 ľubovoľné regulárne množiny $A = \{ a \}$ a $B = \{ b \}$. Avšak množiny $\{a\}$ a $\{b\}$ sú nezrovnateľné. Platí totiž, že
    $\{a\} \not\subseteq \{b\}$ ale aj $\{b\} \not\subseteq \{a\}$. Tým pádom ani $A \not\subseteq B$ a zároveň $B \not\subseteq~A$.
    Z toho vyplýva, že operácia inklúzie $\subseteq$~nie~je \emph{totálnym usporiadaním} na algebre regulárnych množín $A_{RM}$.
\end{mysolution}
