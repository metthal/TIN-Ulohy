...
\begin{mysolution}{subtasks}
	...
	\begin{enumerate}[label=\arabic*)]
		\item Zavedieme počiatočné ohodnotenie $V_{A} = \{\infty\}$ pre každý neterminál $A \in N$.
			Vytvoríme množinu $R_{0} \subseteq P$, do ktorej umiestnime len tie prepisovacie
			pravidlá z $P$, ktoré sú v tvare $A \to w, w \in \Sigma^{*}$.
		\item Nech je $i = 0$.
		\item Pre všetky prepisovacie pravidlá $A \to \alpha \in R_{i}$, vykonávaj nasledovné,
			pokiaľ $R_{i} \not= \varnothing$.
			\begin{enumerate}[label=\roman*)]
				\item Ak $||\alpha|| < ||A||$, tak $V_{A} = V_{A} \cup \{||\alpha||\}$ a do
					$R_{i+1}$ pridaj všetky pravidlá z $P$ v tvare $B \to \beta A\gamma$,
					kde $B \in N$ a $\beta,\gamma \in (N \cup \Sigma)^{*}$, inak len pokračuj
					daľej.
				\item Zvýš $i$ o hodnotu $1$ a pokračuj v cykle.
			\end{enumerate}
		\item Váha gramatiky $||G||$ je potom daná váhou štartovacieho neterminálu $||S||$.
	\end{enumerate}
	Zápis algoritmu v pseudokóde s použitím formálnych matematických zápisov, by vyzeral nasledovne
	\newpage
	\begin{flalign*}
		&\forall A \in N : V_{A} := \{\infty\} & \\
		...
		&||G|| := ||S|| &
	\end{flalign*}
	\subtask V prípade celých čísel je nutné zaviesť dodatočné pojmy pre novú verziu algoritmu.
	\begin{itemize}
		\item \textbf{Index prepisovacieho pravidla} je definovaný podľa bijektívneho zobrazenia
			$idx: P \leftrightarrow I$, kde $I = \{1,...,|P|\}$.
		\item \textbf{Ohodnocovacie pravidlo neterminálu $A$} je definované ako
			$v_{A}$, kde $A \in N$, a $v_{A} \in (I \cup \{0\})$.
	\end{itemize}
	Algoritmus bude rozšírený o priraďovanie ohodnocovacieho pravidla jednotlivým neterminálom.
	Celkový algoritmus je potom nasledovný.
	\begin{enumerate}[label=\arabic*)]
		\item Zavedieme počiatočné ohodnotenie $V_{A} = \{\infty\}$ a počiatočné
			ohodnocovacie pravidlo $v_{A} = 0$ pre každý neterminál $A \in N$.
			Vytvoríme množinu $R_{0} \subseteq P$, do ktorej umiestnime len tie prepisovacie
			pravidlá z $P$, ktoré sú v tvare $A \to w, w \in \Sigma^{*}$.
		\item Nech je $i = 0$.
		\item Pre všetky prepisovacie pravidlá $A \to \alpha \in R_{i}$, vykonávaj nasledovné,
			pokiaľ $R_{i} \not= \varnothing$.
			\begin{enumerate}[label=\roman*)]
				\item Ak $||\alpha|| < ||A||$ a $idx(A \to \alpha) \not= v_{A}$,
					tak $V_{A} = V_{A} \cup \{||\alpha||\}$ a do
					$R_{i+1}$ pridaj všetky pravidlá z $P$ v tvare $B \to \beta A\gamma$,
					kde $B \in N$ a $\beta,\gamma \in (N \cup \Sigma)^{*}$ a $v_{A}$ nastav
					na $idx(A \to \alpha)$.
				\item Inak, ak $||\alpha|| < ||A||$ a $idx(A \to \alpha) = v_{A}$,
					tak $V_{A} = V_{A} \cup \{-\infty\}$ a do
					$R_{i+1}$ pridaj všetky pravidlá z $P$ v tvare $B \to \beta A\gamma$,
					kde $B \in N$ a $\beta,\gamma \in (N \cup \Sigma)^{*}$. Inak
					len pokračuj ďalej.
				\item Zvýš $i$ o hodnotu $1$ a pokračuj v cykle.
			\end{enumerate}
		\item Váha gramatiky $||G||$ je potom daná váhou štartovacieho neterminálu $||S||$.
	...
	\newpage
	\begin{flalign*}
		&\forall A \in N : V_{A} := \{\infty\}, v_{A} := 0 & \\
		&R_{0} := \{A \to w \in P \pipesep w \in \Sigma^{*}\} & \\
		&i := 0 & \\
		&\texttt{while}\ R_{i} \not= \varnothing & \\
		&\hspace{1cm} \texttt{for all}\ (A \to \alpha) \in R_{i}: & \\
		&\hspace{2cm} \texttt{if}\ ||\alpha|| < ||A||\ \texttt{then} & \\
		&\hspace{3cm} \texttt{if}\ v_{A} \not= idx(A \to \alpha)\ \texttt{then} & \\
		&\hspace{4cm} V_{A} := V_{A} \cup \{||\alpha||\} & \\
		&\hspace{4cm} R_{i+1} := R_{i+1} \cup
			\{B \to \beta A \gamma \in P \pipesep B \in N \land \beta,\gamma
			\in (N \cup \Sigma)^{*}\} & \\
		&\hspace{4cm} v_{A} := idx(A \to \alpha) & \\
		&\hspace{3cm} \texttt{else if}\ v_{A} = idx(A \to \alpha)\ \texttt{then} & \\
