...
\begin{mysolution}{subtasks}
	...
	Pokiaľ teda jazyk $\overline{L} \cup L'$ je nutne bezkontextový, tak 
...
\begin{mysolution}{subtasks}
	...
	\begin{flalign*}
		...
		&i := 0 & \\
		&\forall (A \to \alpha) \in R_{i}: & \\
		&\hspace{1cm} \texttt{if}\ ||\alpha|| < ||A||\ \texttt{then} & \\
		&\hspace{2cm} V_{A} := V_{A} \cup \{||\alpha||\} & \\
		&\hspace{2cm} R_{i+1} := R_{i+1} \cup
